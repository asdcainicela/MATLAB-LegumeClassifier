function tabla = procesar_imagen_legumbres(ruta_imagen, ruta_guardado)
    ...
    StdG = zeros(n,1);
    StdB = zeros(n,1);
    RelacionAspecto = zeros(n,1);
    Circularidad = zeros(n,1);
    Compacidad = zeros(n,1);
    ...
    for i = 1:n
        ...
        RelacionAspecto(i) = EjeMayor(i) / EjeMenor(i);
        Circularidad(i) = 4 * pi * Area(i) / (Perimetro(i)^2);
        Compacidad(i) = Area(i) / (EjeMayor(i) * EjeMenor(i));
        
        % Extraer píxeles de color para este objeto
        mascara = etiquetas == i;
        
        % Extraer canales RGB correctamente
        R_channel = x(:,:,1);
        G_channel = x(:,:,2);
        B_channel = x(:,:,3);
        
        pixeles_R = R_channel(mascara);
        pixeles_G = G_channel(mascara);
        pixeles_B = B_channel(mascara);
        
        MeanR(i) = mean(double(pixeles_R));
        MeanG(i) = mean(double(pixeles_G));
        MeanB(i) = mean(double(pixeles_B));
        
        StdR(i) = std(double(pixeles_R));
        StdG(i) = std(double(pixeles_G));
        StdB(i) = std(double(pixeles_B));
    ...
    tabla = table(Objeto, Area, Perimetro, Excentricidad, ...
        ...
        RelacionAspecto, Circularidad, Compacidad);
end